...
function obtenerPersonaje(id, callback) {
  var url = "".concat(API_URL).concat(PEOPLE.replace(':id', id)); //para saber cuando sucede un error, la funcion get nos permite concatenar tambien la funcion fail, la cual nos permitira realizar una accion al momento de que ocurra algun error en la comunicacion con la obtencion de datos a nuestro jQuery, veamos como se implementa

  $.get(url, opciones, callback).fail(function () {
    console.log("sucedio un error al intentar obtener el personaje ".concat(id)); //como podemos observar si nosotros interrumpimos la obtencion de datos el programa arrjora este error y la cadena se detendra en el personaje por obtener, es decir que si el error
