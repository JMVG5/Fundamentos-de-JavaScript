...
function obtenerPersonaje(id, callback) {
  var url = "".concat(API_URL).concat(PEOPLE.replace(':id', id)); //para esto vamos hacer que nuestro primer callback el cual es onResponse, el cual se ejecuta cuando se termina el request, en lugar de ponerlo en otra funcion, lo pondremos directamente dentro de esta funcion 
  //con el callback aun no se hace nada

  $.get(url, opciones, function (data) {
    console.log("hola, yo soy ".concat(data.name)); //para mandar a llamar a nuestra funcion callbak y seriar nuestros datos tendremos que colocar un if que nos ejecutara nuestros parametros si la funcion se esta llamando 
